	 ...
	 * \return int Retorna 0 si el proceso de listado salio bien. Si no un numero menor a 0.
	 */
	int controller_ListProduct(LinkedList* pArrayListEmployee)
	{
		int retorno = -1;
	...
	int controller_ListPArancelaria(LinkedList* pArrayListEmployee)
	{
		int retorno = -1;
		...
		if (pArrayListEmployee != NULL)
		{
			...
			for (int i = 0 ; i < pArrayListEmployee->size ; i++)
			{
				if (nodo->pElement != NULL)
				{
					print_posicionArancelaria (nodo->pElement);
				}
