	...
	/*
	 * \brief Imprime el menu de productos.
	...
	void printMenuProductos (void)
	{
		printf("\n1) Alta de producto.\n"
				...
				"5) Ingresar al menu de posiciones arancelarias.\n"
				"6) Establecer nuevos valores de transporte [MARITIMO].\n"
				"7) Establecer nuevos valores de transporte [AEREO].\n"
				"8) Salir\n");
	}


	/*
	 * \brief Imprime menu de posicion arancelaria.
	 * \return Retorna vacio/void.
	 */
	void printMenuPosicionesArancelarias (void)
	{
		printf("\n1) Alta de posicion arancelaria.\n"
				"2) Baja de posicion arancelaria.\n"
				"3) Modificacion de posicion arancelaria.\n"
				"4) Listar posiciones arancelarias\n"
				"5) Salir\n");
	}

	/*
	 * \brief Imprime menu de despedida.
	 * \return Retorna vacio/void.
	 */
	...
	static int getFloat(float* pResultado)
	{
		...
		if(pResultado != NULL)
		{
			if(getString(buffer,sizeof(buffer))==0 && esFlotante(buffer)==0)
			{
	 ...
	 * \brief Verifica si la cadena ingresada es flotante.
	 * \param cadena char* Cadena de caracteres a ser analizada.
	 * \return int Retorna 0 (vardadero) si la cadena es flotante y < 0 (falso) si no lo es.
	 */
	int esFlotante(char* cadena)
	{
		...
		if(cadena != NULL && strlen(cadena) > 0)
		{
			...
			for( int i=0 ; cadena[i] != '\0'; i++)
			{
				/*if(i==0 && (cadena[i] == '-' || cadena[i] == '+'))
				{
					continue; //NO QUIERO ACEPTAR SIGNO + ni - en mi esFlotante.
				}
				*/
				if(cadena[i] < '0' || cadena[i] > '9' )
				{
					...
					{
						retorno = -2;
						//printf("CARACTER: %c VECES DE PTS: %d",cadena[i],contadorPuntos);
						break;
					}
				}
			}
	...
	static int getNombre(char* pResultado, int longitud)
	{
		...
		if(pResultado != NULL)
		{
			if(	getString(buffer,sizeof(buffer))==0 &&
				esNombreValido(buffer,sizeof(buffer)==0) &&
	...
	static int getDescripcion(char* pResultado, int longitud)
	{
		...
		if(pResultado != NULL)
		{
			if(	getString(buffer,sizeof(buffer))==0 && esDescripcion(buffer,sizeof(buffer)) == 0 && strnlen(buffer,sizeof(buffer))<longitud)
			{
				strncpy(pResultado,buffer,longitud);
				retorno = 0;
			}
	...
	int esNombreValido (char* pCadenaAVerificar, int longitudCadenaAVerificar)
	{
		...
		NO PUEDE SER UN NOMBRE DE 40 LETRAS (MAXIMO 39).
	 ...
	 * \return Retorna 0 si la cadena es valida y -1 si no lo es.
	 */
	int esDescripcion(char* cadena,int longitud)
	{
		...
		if(cadena != NULL && longitud > 0)
		{
			for(i=0 ; cadena[i] != '\0' && i < longitud; i++)
			{
				if(cadena[i] != '.' && cadena[i] != '+' && cadena[i] != '-' && cadena[i] != ' ' && (cadena[i] < 'A' || cadena[i] > 'Z' ) && (cadena[i] < 'a' || cadena[i] > 'z' ) && (cadena[i] < '0' || cadena[i] > '9' ) )
				{
					retorno = -1;
					break;
				}
	...
	int esLargoEmpaqueValido (float largoEmpaque)
	{
		...
		if (largoEmpaque > 0 && largoEmpaque < 1000)
		{
			retorno = 0;
		}
	...
	int esAltoEmpaqueValido (float altoEmpaque)
	{
		...
		if (altoEmpaque > 0 && altoEmpaque < 1000)
		{
			retorno = 0;
		}
	...
	int esAnchoEmpaqueValido (float anchoEmpaque)
	{
		...
		if (anchoEmpaque > 0 && anchoEmpaque < 1000)
		{
			retorno = 0;
		}
	...
	int esVolumenEmpaqueValido (float volumenEmpaque)
	{
		...
		if (volumenEmpaque > 0 && volumenEmpaque < 1000000)
		{
			retorno = 0;
		}
	...
	int esConstanteVolumetricaTxt(char* cadena,int longitud)
	{
		...
		{
			int validacionFloat;
			validacionFloat = esFlotante(cadena);
			...
			if (validacionFloat == 0)
			{
				...
				numeroFloat = atof (cadena);
				validacionConstanteVolumetrica = esConstanteVolumetrica (numeroFloat);
	...
	int esPrecioPorKgTxt (char* precioPorkG)
	{
		...
		{
			int validacionFloat;
			validacionFloat = esFlotante(precioPorkG);
			...
			if (validacionFloat == 0)
			{
				...
				numeroFloat = atof (precioPorkG);
				validacionPrecioPorKG = esPrecioPorKg (numeroFloat);
	...
	int esTipoLicenciaValido (int tipoLicencia)
	{
		...
		if (tipoLicencia != 1 && tipoLicencia != 0)
		{
			retorno = -2;
		}
		else
	...
	int esMetrosCubicosContainer(float mtsCubicos)
	{
		...
		if (mtsCubicos > 0 && mtsCubicos < 100000)
		{
			retorno = 0;
		}
	...
	int esMetrosCubicosContainerTxt(char* cadena,int longitud)
	{
		...
		if(cadena != NULL && longitud > 0)
		{
			...
			if (validacionFloat == 0)
			{
				retorno = -2;
				int numeroFloat;
				int validacionFloatTrans;

				numeroFloat = atof (cadena);
				validacionFloatTrans = esMetrosCubicosContainer(numeroFloat);

				if (validacionFloatTrans == 0)
				{
					retorno = 0;
				}
			}
	...
	int esPrecioPorMtCubicoTxt(char* cadena,int longitud)
	{
		...
		if(cadena != NULL && longitud > 0)
		{
			...
			if (validacionFloat == 0)
			{
				retorno = -2;
				int numeroFloat;
				int validacionMtCubico;

				numeroFloat = atof (cadena);
				validacionMtCubico = esPrecioPorMtCubico (numeroFloat);

				if (validacionMtCubico == 0)
				{
					retorno = 0;
				}
			}
	...
	int calcular_volumenDeEmpaqueCENTIMETROS (float alto, float largo, float ancho, float* volumen)
	{
		...
		if (alto > 0 && largo > 0 && ancho > 0 && volumen != NULL)
		{
			retorno = -2;
	...
	int calcular_volumenDeEmpaqueMETROS (float alto, float largo, float ancho, float* pResultado)
	{
		...
		if (alto > 0 && largo > 0 && ancho > 0 && pResultado != NULL)
		{
			retorno = -2;

			float metrosCubicos;

			/* 	FORMULA DE CM A METRO [AMBAS EQUIVALENTES]
			 *
			 *	(alto / 100) * (largo / 100) * (ancho / 100)
			 *
			 *	(alto * largo * ancho) / (100 * 100* 100)
			 *
			*/

			metrosCubicos = (alto / 100) * (largo / 100) * (ancho / 100);
			*pResultado = metrosCubicos;
	...
	int calcular_baseImponibleTransporteByMar (float volumenMtsCubicos, float precioPorMetroCubico, float* pResultado)
	{
		int retorno = -1;

		{
			/* Regla de 3 simple.
			 * Si 1 mt cubico es x plata.
			 * ¿Cuanta plata es Xmts cubicos?
			 */

			// 1mt cubico = precioPorMetroCubico
			// volumenMtsCubicos = xPrecio

			*pResultado = volumenMtsCubicos * precioPorMetroCubico / 1;
	...
	int calcular_resultado1PrecioAereoCONSTANTEVOL (float volumenCM, float constanteVolumetrica, float precioPorKG, float* pPrecioResultado1)
	{
		...
		if (volumenCM > 0  && constanteVolumetrica > 0 && precioPorKG > 0)
		{

			/* FORMULA
			 *
			 * Resultado1 = Volumen centímetros / CONSTANTE VOLUMETRICA  [MODIFICABLE]
			 * Resultado1 =  Resultado1 * PRECIO POR KG [MODIFICABLE]
			 *
			 */

			float resultado;

			resultado = volumenCM / constanteVolumetrica;
			resultado = resultado * precioPorKG;

			*pPrecioResultado1 = resultado;
			retorno = 0;
		}
	...
	int calcular_resultado2PrecioPORKG (float pesoKg, float precioPorKG, float* pPrecioResultado2)
	{
		...
		if (pesoKg > 0 && precioPorKG > 0 && pPrecioResultado2 != NULL)
		{
		   /*
			* Resultado2 = Peso del articulo * PRECIO POR KG [MODIFICABLE]
			*
			*/

			float resultado;

			resultado = pesoKg * precioPorKG;

			*pPrecioResultado2 = resultado;
			retorno = 0;
		}
